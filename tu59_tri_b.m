...
substitutedExpr1 = simplify(subs(X(1), {s}, {0})); % 自然坐标与总体坐标的关系 ！看阶次
% r+1 = (2*x)^(1/2)
clear X i h
...
substitutedExpr2 = simplify(subs(J_inv(1,1), {s}, {0})); % 雅可比矩阵的奇异性 ！初步判断
...
% 上式化简后，偏u偏x的分母含（1+r）的一次项，而偏u偏y的分母含（1+r）的二次项
% 所以是既有R^(-1)又有R^(-1/2)的奇异阶
